...
class BasicCookingRobot(object):
    ...
    def __init__(self, planner_prefix='BPR'):
        ...
        cook_behaviour = NetworkBehaviour(name='Cook', plannerPrefix=self.__prefix, createLogFiles=True, interruptable=False)
        cook_behaviour.add_precondition(take_order_cond)

        deliver_behaviour = NetworkBehaviour(name='Deliver', plannerPrefix=self.__prefix, createLogFiles=True, interruptable=False)
        ...
        cook_goal = OfflineGoal(name="CookGoal", planner_prefix=cook_behaviour.get_manager_prefix())
        ...
        deliver_goal = OfflineGoal(name="DeliverGoal", planner_prefix=deliver_behaviour.get_manager_prefix())
        deliver_goal.add_condition(customer_interaction_cond)
        deliver_behaviour.add_goal(deliver_goal)

        # set global goal
        self.__global_goal = OfflineGoal(name="MoneyGoal", planner_prefix=self.__prefix)
        self.__global_goal.add_condition(customer_interaction_cond)

        # test_goal
        self.__test_goal = OfflineGoal(name="TestGoal", planner_prefix=self.__prefix)
